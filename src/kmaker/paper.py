...
and the latency are those of PREREGISTRATION.md, section "Forward test".
"""
...
class Simulator:
    ...
    def _fill(self, o: VOrder, reach: float, ts: int, via: str, taker_qty: float) -> Fill | None:
        ...
        o.remaining -= qty
        if o.remaining <= EPS:
            o.cancel_us = ts  # done; the next book posts a fresh quote
        return Fill(
    ...
    def on_taker_order(self, t: TakerOrder) -> list[Fill]:
        ...
        for o in self.orders.get(t.ticker, []):
            if o.side != side or not (o.live_us <= t.created_us < o.cancel_us):
                continue
            if side == "short_yes":
    ...
    def cancel_all(self, ticker: str, now_us: int) -> None:
...
CREATE TABLE IF NOT EXISTS settlements (
    ticker TEXT PRIMARY KEY, result TEXT, settled_us INTEGER, seen_us INTEGER
);
CREATE TABLE IF NOT EXISTS cycles (
    ts_us INTEGER, duration_s REAL, active INTEGER, candidates INTEGER, orders INTEGER,
    taker_orders INTEGER, fills INTEGER, requests INTEGER
);
...
class PaperMaker:
    ...
    def _info(self, m: dict, now_us: int) -> MarketInfo | None:
        ...
        if "ALL" not in self.categories and category not in self.categories:
            return None
        expiry = nm["expected_expiration_us"] or nm["close_us"]
    ...
    def refresh_candidates(self, full: bool) -> None:
        now = time.time()
        now_us = int(now * 1e6)
        if full:
            fresh = {}
            for m in self.client.open_markets():
                info = self._info(m, now_us)
                if info:
                    fresh[info.ticker] = info
    ...
    def scan_step(self) -> None:
        ...
        self.scan_pos = (
            0
            if self.scan_pos + SCAN_BATCH >= len(self.scan_order)
            else (self.scan_pos + SCAN_BATCH)
        )
    ...
    def poll_trades(self) -> list[TakerOrder]:
        started = time.time()
        rows = []
        for t in self.client.recent_trades(int(self.last_trade_poll) - TRADE_OVERLAP_S):
            ...
            if row is not None and row[0] in self.sim.orders:
                rows.append(row)
    ...
    def refresh_books(self) -> list[Fill]:
        fills = []
        delay_us = int(PREREG.live_delay_s * 1e6)
        for i in range(0, len(self.active), 50):
            ...
            for t in batch:
                ...
                if info is None or m is None or m.get("status") not in ("active", "open"):
                    self.sim.cancel_all(t, recv_us)
                    continue
                book = top_of_book(m)
                info.two_sided = book is not None
                info.volume_24h = float(m.get("volume_24h_fp") or info.volume_24h)
                self.known[t] = info
                fills += self.sim.on_book(info, book, recv_us, delay_us)
        return fills

    def check_settlements(self) -> None:
        now_us = int(time.time() * 1e6)
        # every unsettled market, not only past its expected expiration: many close early
        pending = [
            r[0]
        ...
        for i in range(0, len(pending), 50):
            ...
            rows = [
                (t, (m.get("result") or "").lower(), ts_us(m.get("settlement_ts")), now_us)
    ...
    def cycle(self) -> None:
        ...
        if t0 - self.last_full > 6 * 3600:
            self.refresh_candidates(full=True)
        elif t0 - self.last_incremental > 300:
            self.refresh_candidates(full=False)
        self.scan_step()
        self.choose_active(int(time.time() * 1e6))
        fills = []
        taker_orders = self.poll_trades()
        for t in taker_orders:
            fills += self.sim.on_taker_order(t)
        fills += self.refresh_books()
    ...
    def run_forever(self) -> None:
        ...
        while True:
            ...
            try:
                self.cycle()
            except Exception:  # an API hiccup must not stop the forward test; it is logged
